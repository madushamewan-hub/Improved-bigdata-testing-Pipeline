...
def storage(records: List[Dict], db_path: str = ':memory:') -> None:
    ...
    conn.executemany('''INSERT INTO orders_curated VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)''',
                     [(r['event_id'], r['event_time'], r['customer_id'], r['source_system'],
                       float(r.get('amount', 0)), r.get('status'), int(r.get('version', 1)),
                       r.get('checksum'), float(r.get('total_tax', 0)))
                      for r in records])
    conn.close()
...
def run_streaming(orders: List[Dict], db_path: str = ':memory:') -> Dict:
    # For demonstration, the streaming mode reuses the same logic with staged input.
    return run_batch(orders, db_path=db_path)